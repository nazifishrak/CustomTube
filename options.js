...
const DEFAULT_CATEGORIES = {
    ...
    technology: {
      enabled: false,
      keywords: ["tech", "gadgets", "review", "software", "hardware", "AI", "machine learning", "programming", "coding"]
    },
    sports: {
      enabled: false,
      keywords: ["football", "basketball", "soccer", "cricket", "tennis", "match", "tournament", "player", "score"]
    }
  };
    ...
    Object.entries(this.categories).forEach(([name, category]) => {
      ...
      div.innerHTML = `
        ...
        <div class="keyword-input-container">
          <input type="text" class="keyword-input" id="keyword-input-${name}" placeholder="Add new keyword for ${name}" data-category="${name}">
  ...
  setupCategoryEventListeners(categoryName) {
    ...
    keywordsList.addEventListener('click', (e) => {
      if (e.target.classList.contains('remove-keyword')) {
        ...
        this.categories[categoryName].keywords = this.categories[categoryName].keywords.filter(k => k !== keyword);
        this.renderKeywords(categoryName);
